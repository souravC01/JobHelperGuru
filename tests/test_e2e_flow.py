...
def test_full_user_journey_e2e():
    ...
    create_app_res = client.post("/api/applications", json={
        "company": "Stripe",
    ...
    rank_res = client.post("/api/resumes/match", json={
    ...
    opt_res = client.post("/api/resumes/optimize-bullet", json={
    ...
    outreach_res = client.post("/api/resumes/generate-outreach", json={
        "job": job_data["analysis"],
        "resume_id": r1["id"]
    })
    ...
    update_res = client.patch(f"/api/applications/{app_id}", json={
        "status": "Applied",
    ...
    client.delete(f"/api/resumes/{r1['id']}")
    client.delete(f"/api/resumes/{r2['id']}")
